...
func run() -> void:
	...
	expect(cat._postal_outfit.size()==5,"Postal accessories must be available for delivery")
	for part in cat._postal_outfit: expect(not part.visible,"Regular cat must not wear postal accessories")
	...
	for part in cat._postal_outfit: expect(part.visible,"Delivery must show postal accessories")
	...
	for part in cat._postal_outfit: expect(not part.visible,"Postal accessories must disappear after delivery")
