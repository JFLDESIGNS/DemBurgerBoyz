...
func run() -> void:
 ...
 game.stations[0].items=["bun_bottom","tomato"];game.stations[0].patties=[]
 var row=Panel.new();row.position=Vector2(40,40);row.size=Vector2(90,45);row.set_meta("stack_i",1)
 game.get_node("UI/Root").add_child(row);game.stations[0].layer_pool=[row]
 assert(game._try_right_click_build_layer(row.get_global_rect().get_center()))
 assert(game.stations[0].items==["bun_bottom"],"Right-click removes exactly the target layer")
 for cycle in 3:
  var data=game.fryer_baskets[0];data.state="raw";data.cook=0.0;data.shake=0.0
  assert(game._begin_fryer_basket_hold(0));game._fryer_auto_mode="cook"
  for i in 480:game._update_held_fryer_basket(1.0/60.0)
  assert(game.fryer_held_index==-1 and game.fryer_baskets[0].state=="done")
  assert(game._begin_fryer_basket_hold(0));game._fryer_auto_mode="shake"
  for i in 480:game._update_held_fryer_basket(1.0/60.0)
  assert(game.fryer_held_index==-1 and game.fryer_baskets[0].state=="empty","Basket remains usable after repeated cycles")
 var ghost=load("res://scripts/customer.gd").new()
 ghost.setup(order,Color.WHITE,300.0,0,0,0,-1,{},true)
 game.customers_root.add_child(ghost);game.customers.append(ghost);ghost.set_meta("mp_net_id",98001);ghost.set_meta("burger_in_flight",true)
 var ticket=Control.new();game.get_node("UI/Root").add_child(ticket);game.tickets[ghost]=ticket
 game._mp_force_remove_customer(ghost)
 assert(not game.tickets.has(ghost) and not game.customers.has(ghost),"Stuck serve animation cannot retain ghost ticket")
 ghost.set_meta("burger_in_flight",false);ghost.queue_free()
 print("RELIABILITY_LOCAL_OK")
 game.queue_free()
